    ...
    if(ImGui AND "${CMAKE_CURRENT_SOURCE_DIR}" STREQUAL "${ImGui_SRCDIR}/examples")
        if(ImGui_3RDPARTY)
            ...
            if(NOT TARGET_DEPENDENCIES)
                message(STATUS "Skip ${TARGET} binding because not all dependencies found")
                return()
            endif()
        endif()
        add_library(${TARGET} INTERFACE)
        ...
        foreach(HEADER ${TARGET_HEADERS})
            target_sources(${TARGET}
                INTERFACE
                    $<BUILD_INTERFACE:${ImGui_SRCDIR}/examples/${HEADER}>
                    $<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}/imgui/${HEADER}>)
            install(FILES ${ImGui_SRCDIR}/examples/${HEADER}
                DESTINATION ${CMAKE_INSTALL_INCLUDEDIR}/imgui)
        endforeach()
        foreach(SOURCE ${TARGET_SOURCES})
            target_sources(${TARGET}
                INTERFACE
                    $<BUILD_INTERFACE:${ImGui_SRCDIR}/examples/${SOURCE}>
                    $<INSTALL_INTERFACE:${CMAKE_INSTALL_DATAROOTDIR}/imgui/${SOURCE}>)
            install(FILES ${ImGui_SRCDIR}/examples/${SOURCE}
                DESTINATION ${CMAKE_INSTALL_DATAROOTDIR}/imgui)
        endforeach()
        target_include_directories(${TARGET} INTERFACE
            $<BUILD_INTERFACE:${ImGui_SRCDIR}/examples>
            $<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}/imgui>)
    ...
    elseif(ImGui)
        ...
        if(${TARGET} IN_LIST ImGui_FIND_COMPONENTS)
            ...
            if(ImGui_3RDPARTY)
                ...
                message(STATUS "Please manualy link 3-th party dependencies for ${TARGET} binding")
            endif()
        endif()
    endif()
endfunction()
...
function(imgui_example SUBDIRECTORY)
    if(NOT ImGui_EXAMPLES OR NOT ImGui_BINDINGS OR NOT ImGui_3RDPARTY)
        return()
    endif()
    cmake_parse_arguments(EXAMPLE "" "" "TARGETS;BINDINGS" ${ARGN})
